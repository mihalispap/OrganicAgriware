 ...
 */
function bizreview_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'bizreview_form_submit';
}


function bizreview_form_submit($form, &$form_state) {
  // Import BizReview database file.
  $bizreview_file = dirname(__FILE__) . '/bizreview.sql';
  $success = import_bizreview($bizreview_file);
...
/**
 * Imports the BizReview database file.
 *
 * @see demo_reset().
 */
function import_bizreview($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open BizReview file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
